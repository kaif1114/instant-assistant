...
export function ChatPreview() {
  ...
  // Show messages one by one when component is in view
  useEffect(() => {
    if (isInView) {
      const showMessage = (index: number) => {
        if (index < messages.length) {
          setVisibleMessages(prev => [...prev, messages[index]]);
          setTimeout(() => showMessage(index + 1), messages[index].delay * 1000);
        }
      };
      showMessage(0);
    }
  ...
  }, [visibleMessages]); // Watch visibleMessages instead of messages
            ...
            >
              <div className="chat-preview rounded-[20px] overflow-hidden w-full max-w-md mx-auto lg:mx-0">
                <Card className="w-full h-[700px] flex flex-col rounded-[20px] overflow-hidden shadow-md">
                  ...
                  >
                    <h2 className="text-xl font-bold text-white text-center w-full py-2">
                      Instant Assistant
                    </h2>
                  </CardHeader>
                      ...
                      <div className="p-4 space-y-4" ref={scrollAreaRef}>
                        <div className="flex flex-col items-center text-center mb-6">
                          <Avatar className="w-16 h-16 bg-black rounded-full flex items-center justify-center mb-4">
                            <AvatarImage src="/logo_dark.png" alt="AI Assistant" />
